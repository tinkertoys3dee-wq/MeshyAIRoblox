...
export class JobRunner {
  ...
  async #runImagePreview(job: Job): Promise<void> {
    ...
    if (!image) {
      ...
      image = await this.#images.generateAccessoryReference(job.filteredPrompt, job.id);
  ...
  async #runTextTo3D(job: Job): Promise<void> {
    ...
    if (!previewTaskId) {
      ...
      previewTaskId = await this.#meshy.createTextPreview(job.filteredPrompt);
    ...
    if (!finalTaskId) {
      ...
      finalTaskId = await this.#meshy.createTextRefine(previewTaskId, job.filteredPrompt);
  ...
  async #runImageTo3D(job: Job): Promise<void> {
    ...
    if (!finalTaskId) {
      ...
      finalTaskId = await this.#meshy.createImageTo3D(source.imageArtifact, "image/png", job.filteredPrompt);
      await this.#mergeOutput(job.id, { meshyFinalTaskId: finalTaskId });
    }
