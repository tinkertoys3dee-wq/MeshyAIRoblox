...
Avatar-sourced generation (`AVATAR_TO_3D`) is a single-step paid path like Direct Forge, not the two-step submit-then-approve flow custom-image upload uses — there is no intermediate `ENTITLED`/`IMAGE_MODERATION`/`IMAGE_READY` stage, since there is nothing for the player to approve. Its source image also arrives differently than the diagram above shows for uploads: rather than the Roblox server forwarding a numeric asset ID for Railway to resolve, the Roblox server itself calls Roblox's public Thumbnails web API (`AvatarThumbnailService`, `GET https://thumbnails.roblox.com/v1/users/{avatar-headshot|avatar-bust|avatar}`) and forwards Railway the resulting HTTPS image URL directly as `sourceImageUrl` on `POST /v1/jobs`. This is deliberately not `Players:GetUserThumbnailAsync`, which only ever returns an `rbxthumb://` Content reference the Roblox engine resolves internally for on-screen display and that no external HTTP client, Railway included, can fetch; the client-side live avatar preview in the wizard uses that engine-resolved reference directly, while the server-side job input uses the real URL. `Config.Products.AvatarConversion` reuses `ImageConversion`'s cost and price, since the underlying Meshy operation is identical.
