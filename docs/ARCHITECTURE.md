...
`RetentionService` owns the free four-step makeover journey: open the makeover studio, wear a starter look, change one item, and save the outfit. Before it, the client renders one visual `PICK → CHANGE → SAVE` tutorial and the server persists `profile.tutorialVersion` through an allowlisted `CompleteTutorial` choice. Avatar Lab then derives the active guide stage from server-authored makeover/journey state, hides unrelated controls, and highlights the exact Wear, Try on, or Save control; dismissal is local and never loses progress. The client may report only `OPEN_STUDIO`; catalog/community application and outfit saving advance from successful server branches. `MakeoverService` owns the 90-second session, today’s theme, category/personalization score, one daily Creator-XP award, and granular makeover funnel. The client resolves three themed sets from Roblox’s live on-sale catalog; `CatalogService:ApplyLook` validates every ID/type pairing and applies the set in one `HumanoidDescription` update and one undo checkpoint. Catalog search/try-on does not request inventory-read access. Onboarding steps three and four remain server-authored from real `OPEN_STUDIO` and `TRY_STYLE` actions. Journey token grants use `bonusTokens`, so they can buy a Priority Pass but cannot fund OpenAI or Meshy work.

Create owns a separate session-scoped personal-UGC guide for accounts without a completed generation and for players who explicitly reopen it. The opening tutorial routes `MAKE_OWN` into this guide instead of back into makeover help. Its scrollable `DESCRIBE → CONFIRM → FIT → WEAR` rail, private-model/publishing explanation, animated next-action banner, and highlighted prompt precede the creation controls; the unrelated makeover card is withheld while this guide is active. `CreatorGuide` analytics accepts only an enumerated action and never records prompt text.
