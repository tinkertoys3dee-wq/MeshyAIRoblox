...
Avatar-sourced generation (`AVATAR_TO_3D`) is a single-step paid path like Direct Forge, not the two-step submit-then-approve flow custom-image upload uses — there is no intermediate `ENTITLED`/`IMAGE_MODERATION`/`IMAGE_READY` stage, since there is nothing for the player to approve. Its source image also arrives differently than the diagram above shows for uploads: rather than the Roblox server forwarding a numeric asset ID for Railway to resolve, the Roblox server sends only the player's chosen `avatarView` (headshot/bust/full body) on `POST /v1/jobs`, and Railway resolves that plus the `playerUserId` already stamped on every job (see `BackendService:CreateJob`) into a real fetchable image URL itself, via `GET https://thumbnails.roblox.com/v1/users/{avatar-headshot|avatar-bust|avatar}?userIds={playerUserId}`. The Roblox server cannot make that call itself: `HttpService` is blocked from reaching Roblox-owned domains at the platform level, confirmed live ("HttpService is not allowed to access that Roblox resource") when an earlier version of this feature tried exactly that from a Roblox-side `AvatarThumbnailService`, which existed only briefly before being removed for this reason. This is separately why the client never uses `Players:GetUserThumbnailAsync` for its own live preview either — that call only ever returns an `rbxthumb://` Content reference the Roblox engine resolves internally for on-screen display, never a URL any external HTTP client could fetch — the client instead builds that `rbxthumb://` string directly (see `avatarThumbnail()` in `App.luau`), with no HTTP call and no relation to the server-side job's image resolution. `Config.Products.AvatarConversion` reuses `ImageConversion`'s cost and price, since the underlying Meshy operation is identical.
...
Forge Tokens are earned by watching a Roblox Rewarded Video ad (`Class.AdService`), which Roblox splits across client availability checks and server playback/reward creation. Each ad needs an explicit player activation and is rechecked before playback; the AFK lounge may poll availability but never chains ads. The actual one-token grant arrives asynchronously through `MarketplaceService.ProcessReceipt`. A second grant handler gives 70 tokens for the separate `TokenPack70` product. Passive AFK and seven-day login rewards use `bonusTokens` and remain Priority-Pass-only.
...
- First makeover progress, daily Style Sprint score/completion, Creator XP/level statistics, runway participation, daily first-round/Encore reward state, and idempotent weekly payout state;
...
## First makeover, Style Sprint, Creator progression, and Forge Runway

`RetentionService` owns the free four-step makeover journey: open the makeover studio, wear a starter look, change one item, and save the outfit. Before it, the client presents only three concrete start choices—Style, Create, or Play—and the server persists `profile.tutorialVersion` through an allowlisted `CompleteTutorial` choice. Choosing Style starts the contextual makeover guide; Avatar Lab then derives its active stage from server-authored makeover/journey state, hides unrelated controls, and highlights the exact Wear, Try on, or Save control. Dismissal is local and never loses progress. The client may report only `OPEN_STUDIO`; catalog/community application and outfit saving advance from successful server branches. `MakeoverService` owns the 90-second session, today’s theme, category/personalization score, one daily Creator-XP award, and granular makeover funnel. The client resolves three themed sets from Roblox’s live on-sale catalog; `CatalogService:ApplyLook` validates every ID/type pairing and applies the set in one `HumanoidDescription` update and one undo checkpoint. Catalog search/try-on does not request inventory-read access. Onboarding steps three and four remain server-authored from real `OPEN_STUDIO` and `TRY_STYLE` actions. Journey token grants use `bonusTokens`, so they can buy a Priority Pass but cannot fund OpenAI or Meshy work.

The client shell is organized around Home plus three jobs: Make Item, Style Avatar, and My Items. Community discovery, avatar art, Settings, and Play live one level below Home. Simple Create exposes a single Direct Forge prompt, examples, one price-bearing action, and an optional latest-job status; advanced methods, tokens, queue details, rewards, and specialist controls are disclosed only after the player requests them. Create retains a session-scoped `DESCRIBE → CONFIRM → FIT → WEAR` guide behind **How It Works**, but choosing Create never stacks that second lesson on top of the start choice. `CreatorGuide` and `HomeChoice` analytics accept only enumerated values and never record prompt text.
...
`RunwayService` synchronizes a server-local style round without creating Workspace instances or changing Lighting. Players join during intermission/styling, mark a look ready, inspect live character appearances inside client-only `WorldModel` viewports, and cast at most one non-self vote. It is an optional next action after the fast solo makeover, not an onboarding gate. In-game equip/unequip/try-on/reset routes freeze for ready entrants during voting/results, so a displayed look cannot change after ballots open. Departed entrants and their votes are removed safely. Equal vote totals use a server-authored random tie break rather than lock-in speed. Results grant participation/placement XP, the first daily `bonusTokens` reward, daily-quest progress, and weekly spotlight points. `profile.runway.dailyRounds` and `dailyEncoreCompleted` make the visible two-round Daily Encore UTC-scoped and one-time, with rewards applied in the same result mutation before the profile save. The weekly top-three payout reads an `OrderedDataStore`; per-profile idempotency permits overlapping servers, and the global completion marker advances only after every winner mutation succeeds.
