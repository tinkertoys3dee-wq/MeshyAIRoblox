...
- Every account sees the current version of a single-screen, skippable **How to Play** tutorial once. It previews `PICK → CHANGE → SAVE`; its primary action starts the free 90-second makeover, while its secondary path explicitly previews personal UGC as `DESCRIBE → CONFIRM → FIT → WEAR`. It states that catalog preview is free, AI generation is paid, generation first creates a private Forge model, and Roblox publishing is optional and separate. The tutorial version is persisted separately from legacy onboarding progress so this materially clearer edition reaches existing players once.
