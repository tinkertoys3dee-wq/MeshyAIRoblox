...
- A new player starts with a free, four-step **First Look** journey: open the studio, try an accessory, lock a look into Forge Runway, and finish the round. It teaches the avatar/catalog/social loop before asking for an AI purchase. Each step grants a small `bonusTokens` reward and completing the journey grants Creator XP; neither reward can pay provider-backed generation cost.
- Forge Runway is the primary repeatable social loop: server-synchronized intermission, styling, voting, and results phases built entirely in UI/isolated avatar viewports. A populated round lasts six minutes; low-population servers skip meaningless voting while still granting participation credit. It never builds a stage in Workspace or changes world Lighting.
