...
- Each policy flag applies only to its matching Roblox feature. `IsEligibleToPurchaseCommerceProduct` governs real-world Commerce Products and never gates ordinary developer products, passes, catalog assets, Robux transfers, or rewarded ads. Content-sharing, subscription, and paid-trading restrictions remain enforced server-side where those exact capabilities are used.
...
- Every account sees the current version of a single-screen start choice once. It asks only **Style my avatar**, **Make my own item**, or **Play games**, with one short sentence and an honest `FREE`/paid label for each option. Players learn the chosen tool in context instead of reading a product manual before acting. The tutorial version is persisted separately from legacy onboarding progress so this materially simpler edition reaches existing players once.
- A new player starts with a free, four-step **First Makeover** journey: open Avatar Lab, apply one of three live-catalog starter looks in one tap, change one item, and save the outfit. Guided mode hides unrelated tabs, filters, bookmarks, purchase links, and secondary avatar tools; a three-stage indicator, animated arrow, and pulsing gold outline point to the next real control. The full look applies atomically and opens a before/now reveal. Completion offers avatar art, optional Forge Runway, another sprint, or a contextual “Forge a missing piece” path. Each journey step grants a small `bonusTokens` reward and completion grants Creator XP; Style Sprint adds one bounded daily XP reward. None can pay provider-backed generation cost.
...
- The Avatar Lab supports Roblox catalog search, free try-on, platform purchase prompts, avatar saving, and reset. Simple Mode leads with only **Find items** and **My avatar**, seven common categories, and a single optional filter control. Bookmarks, custom items, the daily challenge, extended categories, style sparks, creator search, price, and sorting remain progressively disclosed.
...
- Arcade physics, actors, hazards, and pointer controls use one logical canvas coordinate space. They never feed `UIScale`-multiplied `AbsoluteSize` or unscaled screen-pointer positions back into offset layouts, and every playable object stays inside the visible canvas at every interface scale and supported viewport.
- The Studio opens on a permanent Home screen with three large job choices: **Create**, **Style**, and **Play**. Primary navigation contains only Home, Make Item, Style Avatar, and My Items; community browsing, avatar images, and settings live behind Home's **More tools** control, while Play reveals Arcade, Runway, and AFK only after it is chosen. In Simple Mode, Create is one full-width prompt with examples, one price-bearing action, an optional latest-job status, and a **More creation options** control. Tokens, payment toggles, method cards, queue capacity, streaks, quests, outfits, fitting, pricing, and sharing remain available in Advanced Mode without competing with a beginner's first action.
...
Implemented in the current checkpoint: all creation flows with tiered reference-image quality, receipt-safe products, a separate 70-token store pack, rewarded-ad tokens, a seven-day login calendar, a server-authoritative AFK lounge, the free 90-second makeover and live starter looks, six-minute Forge Runway rounds with a two-round Daily Encore, Creator levels/titles, a retry-safe weekly Runway Spotlight, free daily style/runway/arcade quests, Railway generation and validation, fitting/equip/unequip/publish services, live avatar previews that preserve existing accessories, shared worn-item management in Catalog and Discover, Plus-transfer copies, creator search/sort/reactions, immediate gallery deletion, full-resolution image links, completion celebrations, accessibility settings, keyboard/gamepad focus, responsive scrollbar gutters, sound hooks, and analytics instrumentation.
