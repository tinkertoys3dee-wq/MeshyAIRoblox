...
- Every account sees the current version of a single-screen, skippable **How to Play** tutorial once. It teaches `PICK → TWEAK → SAVE`; its primary action starts the free 90-second makeover, its secondary action opens custom UGC creation, and it states that catalog preview is free while AI generation is paid. The tutorial version is persisted separately from legacy onboarding progress so this materially clearer edition reaches existing players once.
- A new player starts with a free, four-step **First Makeover** journey: open Avatar Lab, apply one of three live-catalog starter looks in one tap, change one item, and save the outfit. The full look applies atomically and opens a before/now reveal. A persistent objective card and timer guide the next action. Completion offers avatar art, optional Forge Runway, another sprint, or a contextual “Forge a missing piece” path. Each journey step grants a small `bonusTokens` reward and completion grants Creator XP; Style Sprint adds one bounded daily XP reward. None can pay provider-backed generation cost.
