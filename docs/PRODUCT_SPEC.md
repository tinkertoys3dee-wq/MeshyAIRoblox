...
- Every account sees the current version of a single-screen, skippable **How to Play** tutorial once. It previews `PICK → CHANGE → SAVE`; its primary action starts the free 90-second makeover, its secondary action opens custom UGC creation, and it states that catalog preview is free while AI generation is paid. The tutorial version is persisted separately from legacy onboarding progress so this materially clearer edition reaches existing players once.
...
- Simple Mode puts the free makeover first and the idea box second. Method details and the token/payment card come later; copy uses concrete verbs and avoids provider, topology, moderation-pipeline, and texture-resolution jargon. Custom generation is introduced contextually after a saved look or directly by explicit choice; the paid button and nearby reassurance show the price before interaction and state that the native Roblox confirmation appears before charging.
