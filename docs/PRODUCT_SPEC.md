...
Implemented in the current checkpoint: both creation flows, receipt-safe developer products, Railway provider pipeline, GLB validation, fitting/equip/publish services, individual player profiles, Plus-transfer copies, discovery/reactions/trending score, catalog lab, policy gates, group/streak benefits, responsive UI, sound hooks, and analytics instrumentation.

Creator-owned activation still required: group ID, developer product IDs, Avatar Creation Token IDs and prices, sound asset IDs and permissions, Railway/PostgreSQL deployment, Creator Hub/Open Cloud secrets, maximum player setting, and live Studio/device testing.
