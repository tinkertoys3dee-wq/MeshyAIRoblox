...
|---|---|---|
| Activation / play-through | start-choice completion; recurring Home choice mix; custom-guide open/prompt-focus/submit; starter looks shown; apply click/request/success/visible; personalization; outfit save; next action; first creation attempt | `VisualTutorialShown`, `VisualTutorialCompleted` (`choice`), `HomeChoice` (`CREATE`/`STYLE`/`PLAY`), `CreatorGuide`, onboarding funnel, `FirstLookGuide`, `MakeoverAction`, `StarterLookApplied`, `StyleSprintStarted`/`Completed`, `FirstLookStepCompleted`, `FirstLookJourneyCompleted`, `makeover` and `creator_journey` funnels, generation funnel |
...
- First-session choice: `VisualTutorialShown` → `VisualTutorialCompleted`, split by the enumerated `START_FREE`, `MAKE_OWN`, or `SKIP` choice. The one-screen chooser is versioned, so the completion event fires once per materially new edition, including for existing accounts. `HomeChoice` then measures the same three intentions on ordinary, repeat Home visits without reusing the one-time onboarding event.
- Personal UGC guide: `CreatorGuide.IMPRESSION` → `PROMPT_FOCUSED` → `SUBMITTED`; compare explicit `OPENED` and `DISMISSED` counts to identify whether the explanation is discoverable, useful, or still too obstructive. These events contain only the enumerated action and never the player's prompt.
...
- `ForgeEligibilitySnapshot.policyLookupSucceeded` and `.commerceProductAllowed` are diagnostics for policy lookup health and real-world Commerce Products only. Never interpret `commerceProductAllowed` as eligibility for developer products, passes, catalog prompts, Robux transfers, or rewarded ads.
- First makeover: studio opened → starter look applied → one distinct item changed → outfit saved. `MakeoverAction` separately distinguishes contextual guide stages/dismissal, looks displayed, apply clicked, request started, server success, avatar visibly updated, failures, and the chosen next action.
