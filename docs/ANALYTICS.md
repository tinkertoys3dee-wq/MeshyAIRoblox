...
|---|---|---|
| Activation / play-through | tutorial view-to-choice; custom-guide impression/open/prompt-focus/submit; starter looks shown; apply click/request/success/visible; personalization; outfit save; next action; first creation attempt | `VisualTutorialShown`, `VisualTutorialCompleted` (`choice`), `CreatorGuide`, onboarding funnel, `FirstLookGuide`, `MakeoverAction`, `StarterLookApplied`, `StyleSprintStarted`/`Completed`, `FirstLookStepCompleted`, `FirstLookJourneyCompleted`, `makeover` and `creator_journey` funnels, generation funnel |
