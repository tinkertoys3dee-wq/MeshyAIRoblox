...
|---|---|---|
| Activation / play-through | tutorial view-to-choice; free-look impression/click/success; First Look step and round completion; first creation attempt | `VisualTutorialShown`, `VisualTutorialCompleted` (`choice`), onboarding funnel, `FirstLookGuide`, `CommunityTryOnAttempted`/`Succeeded`/`Failed`, `FirstLookStepCompleted`, `FirstLookJourneyCompleted`, `creator_journey` funnel, generation funnel |
