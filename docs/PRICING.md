...
`Config.Ads.MaxAdWatchesPerDay` (20) bounds both grind and repeated `AdService:ShowRewardedVideoAdAsync` server calls per player per UTC day; it is a request cap, not a completion guarantee, so a skipped or unfilled ad still counts against it — acceptable given Roblox reports rewarded-video completion rates above 90%. At 20 ads/day and 1 token/ad, funding one 1,590-token generation entirely through play takes roughly 80 days of maxing out the daily cap — a real but honest cost, not a trivial grind, which is the intended trade for skipping a 159 R$ purchase.
