...
- `profile.tokens` — backed by an actual rewarded ad view or a direct Robux purchase of `AdRewardTokens`. May fund a generation product (Direct Forge, every reference-image quality tier, Image → 3D Conversion) or the Priority Pass.
...
The Arcade (Flappy Bird, Dino Runner, Color Switch — see `src/Client/UI/Games/` and `src/Client/UI/GamesHub.luau`) is a client-only session-time feature, not part of the generation economy, but `Config.Arcade.WeeklyRewards` (450/250/100 `bonusTokens` for 1st/2nd/3rd each game, every week) puts real free tokens on the other end of it. That's safe for the same structural reason achievement tokens are: `bonusTokens` can only ever buy the Priority Pass, never a generation product, no matter how large these numbers are or how many games one player tops — see `CommerceService:BeginIntentWithTokens`. This is a game-balance/engagement choice, not something `NonEarningTokenBudget` needs to account for (that budget is specifically achievement pacing).
