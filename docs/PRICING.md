...
The four-step makeover journey grants 2 `bonusTokens` per step plus 12 on completion (20 total) and the first Style Sprint completed each UTC day grants 40 Creator XP. The first completed runway round of each UTC day grants 5, the second completed round finishes the Daily Encore for 10 `bonusTokens` plus 50 Creator XP, and the free daily try-style/runway/arcade quest trio grants 3 each plus a 6-token clear bonus. All token rewards here are intentionally `bonusTokens`; even a player completing every free loop can only redeem the negligible-variable-cost Priority Pass, never an OpenAI/Meshy generation. Creator XP, makeover scores, level titles, votes, and weekly Spotlight awards have no currency value. Their purpose is activation, session depth, and D1/D7 return—not disguising a paid AI operation as a grind reward.
...
The Arcade (Flappy Bird, Dino Runner, Color Switch, Neon Snake, Block Stacker, Brick Breaker, Reflex Tap, Prism Merge, Meteor Rush, Sky Hopper, Echo Match — see `src/Client/UI/Games/` and `src/Client/UI/GamesHub.luau`) is a client-only session-time feature, not part of the generation economy, but `Config.Arcade.WeeklyRewards` (150/83/33 tokens for 1st/2nd/3rd each game, every week — a 15% reduction from 176/98/39, rounded to whole tokens) puts real free tokens on the other end of it. Unlike every other free grant in this file, these are credited to `profile.tokens` — the same fully-spendable pool ad rewards and Robux purchases fund, generations included — not the spend-restricted `bonusTokens` pool. That is a deliberate, studio-owner-approved exception to the "only ever gain money" rule everything else here follows, not an oversight: worst case is every rank in every one of the 11 games won by a different player every week, 2,926 tokens total, worth roughly 2.4 generations (~$0.73) at the pricing above — a slightly higher ceiling than the 176/98/39 era despite the per-game cut, since Sky Hopper and Echo Match add two more games to multiply against. It's accepted as an engagement cost in the same category as ad-funded tokens, sized small enough to absorb even at the worst-case ceiling — not something `NonEarningTokenBudget` needs to account for (that budget is specifically achievement pacing, and achievement/daily-check-in/win-back grants are still `bonusTokens`-only).
