...
`Config.Products.AvatarConversion` (159 R$, `Id = 0` placeholder pending Creator Hub setup, same convention as every other unconfigured ID in this file) is the same Meshy image → Smart Topology model operation and provider cost as `ImageConversion` above — only the source image differs. Instead of an uploaded asset or an OpenAI-generated reference, `AvatarThumbnailService` fetches the player's own current Roblox avatar render server-side, from Roblox's public Thumbnails web API (`https://thumbnails.roblox.com/v1/users/{avatar-headshot|avatar-bust|avatar}`), and hands the backend that URL instead of an uploaded asset ID. This is deliberately not `Players:GetUserThumbnailAsync`, which only ever returns an `rbxthumb://` reference the Roblox engine resolves internally for on-screen display — never a URL an external service can download — so the client-side live preview and the server-side job-input fetch use two different mechanisms for the same underlying render. Because the operation and cost are identical to `ImageConversion`, it is priced the same and added to `GENERATION_TOKEN_PRODUCTS` so Forge Token payment prices against the same worst-case-ad formula as every other model-producing product (see "Forge Tokens" below) rather than the much cheaper non-generation markup.
...
- `profile.tokens` — backed by an actual rewarded ad view or a direct Robux purchase of `AdRewardTokens`. May fund a generation product (Direct Forge, every reference-image quality tier, Image → 3D Conversion, Avatar → 3D Conversion) or the Priority Pass.
- `profile.bonusTokens` — free grants with no revenue behind them at all: achievements (`AchievementService`), the daily check-in bonus (`RetentionService`), invites, group join. May **only ever** buy the Priority Pass.
...
Re-derive `TokenGrantPerPurchase`, `TokenPurchaseMarkup`, and `WorstCaseAdRevenuePerAd` if real eCPM data moves meaningfully from the ~0.12–0.44 Robux figure above, the same way `IMAGE_QUALITY_PROVIDER_COST_USD` gets revisited against measured OpenAI spend — and if it does, move `WorstCaseAdRevenuePerAd` to the new *worst* case, not the new average.
