...
Outside paid generation, the experience has a free 90-second Style Sprint makeover, three live-catalog starter looks, a saved-outfit journey, server-synchronized six-minute Forge Runway rounds, a visible two-round Daily Encore, Creator XP/levels/titles, a cross-server weekly spotlight, daily style/runway/arcade quests, Roblox catalog try-on, an arcade, and an AFK token lounge. These systems are designed to improve qualified play-through and repeat sessions without subsidizing provider-backed AI jobs; routine free rewards use the Priority-Pass-only `bonusTokens` balance.
